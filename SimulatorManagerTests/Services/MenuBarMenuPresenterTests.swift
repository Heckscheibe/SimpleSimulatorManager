...
/// Everything that drives the application's one real status item and its one real panel.
///
/// Serialized here rather than on the suites inside it: two suites that are each serialized on their
/// own still run alongside each other, and one of them opening the panel while the other is closing
/// it makes both fail for reasons that have nothing to do with the code under test.
@Suite("Menu bar panel integration", .serialized)
@MainActor
struct MenuBarPanelIntegrationTests {}
