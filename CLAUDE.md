...
- **`MenuPanelViewModel`**: the open path and keyboard state (selection, arrow movement, the two-step guard on destructive rows). The path is kept as identifiers resolved against a freshly built tree on every render, which is what lets FSEvents-driven changes land in an open panel — and in any flyout hanging off it
...
- **Submenus are flyouts, not drill-down**: hovering a submenu row opens its children in a window beside the panel, and the chain nests. The open path *is* the chain — one flyout per `pathIdentifiers` entry — so the keyboard model from the drill-down did not have to change, only the rendering
- **`MenuFlyoutPresenter`** (behind `MenuFlyoutPresenting`) owns the windows, **`MenuFlyoutController`** owns the timing, **`MenuFlyoutGeometry`** owns the arithmetic. Splitting them is what makes placement and the safe triangle testable without putting anything on screen. A flyout is a `.nonactivatingPanel` ordered `.above` its parent and **never made key**: the panel dismisses itself when it loses key, and the search field has to keep first responder or typing stops filtering. It is ordered in transparent and faded in once its contents report a size, because SwiftUI does not lay out a view that was never in a visible window
- **`MenuBarMenuPresenter`** (behind `MenuBarMenuPresenting`): opens and closes the panel by clicking the status item's button, found by class name in `NSApp.windows`, and hands out the panel's window for the flyouts to hang off. The one place that depends on `MenuBarExtra` internals — keep it isolated there
...
- Both the panel and a flyout draw their rows through `MenuNodeRowsView` and `MenuRowHandlers`. They are separate windows and cannot share a view hierarchy, so the behaviour is passed in rather than reimplemented — a change to how a row hovers or activates belongs there, not in one of the two callers
...
- `MenuBarPanelIntegrationTests` runs inside the host app against the **real** status item and panel, so it catches a SwiftUI change that would otherwise leave the shortcut silently doing nothing, or a macOS release where a flyout dismisses the panel it hangs off. Every suite that drives the panel is nested inside it, because that outer `.serialized` is what stops two of them opening and closing the one panel at the same time
